...
void FormulaList::generateBitmaps(const char *path)
{
  ...
  {
    FTextStream t(&f);
    if (DOXY_CONFIG_GET_BOOL("LATEX_BATCHMODE")) t << "\\batchmode" << endl;
    t << "\\documentclass{article}" << endl;
    t << "\\usepackage{epsfig}" << endl; // for those who want to include images
    const char *s=DOXY_CONFIG_GET_LIST("EXTRA_PACKAGES").first();
    while (s)
    {
      t << "\\usepackage{" << s << "}\n";
      s=DOXY_CONFIG_GET_LIST("EXTRA_PACKAGES").next();
    }
  ...
  if (pagesToGenerate.count()>0) // there are new formulas
  {
    ...
    QCString latexCmd = DOXY_CONFIG_GET_STRING("LATEX_CMD_NAME");
    ...
    for (;(pagePtr=pli.current());++pli,++pageIndex)
    {
      ...
      int zoomFactor = DOXY_CONFIG_GET_INT("FORMULA_FONTSIZE");
