...
{
  static QStrList &exclSyms = DOXY_CONFIG_GET_LIST("EXCLUDE_SYMBOLS");
...
{
  bool vhdlOpt = DOXY_CONFIG_GET_BOOL("OPTIMIZE_OUTPUT_VHDL");
...
{
  static QCString outputLanguage = DOXY_CONFIG_GET_ENUM("OUTPUT_LANGUAGE");
...
{
  static bool inlineSources = DOXY_CONFIG_GET_BOOL("INLINE_SOURCES");
  ol.pushGeneratorState();
    ...
    const QCString &text,MemberSDict *members,bool /*funcOnly*/)
{
  ...
  static bool rtfSourceCode   = DOXY_CONFIG_GET_BOOL("RTF_SOURCE_CODE");
  static bool sourceBrowser   = DOXY_CONFIG_GET_BOOL("SOURCE_BROWSER");
  static bool refLinkSource   = DOXY_CONFIG_GET_BOOL("REFERENCES_LINK_SOURCE");
  ol.pushGeneratorState();
  if (members)
  {
    members->sort();
...
{
  if (DOXY_CONFIG_GET_BOOL("REFERENCED_BY_RELATION"))
  {
...
{
  if (DOXY_CONFIG_GET_BOOL("REFERENCES_RELATION"))
  {
...
QCString abbreviate(const char *s,const char *name)
{
  ...
  QStrList &briefDescAbbrev = DOXY_CONFIG_GET_LIST("ABBREVIATE_BRIEF");
