   ...
   : Definition((QCString)p+nm,1,1,nm)
{
  ...
  acquireFileVersion();
  m_subGrouping=DOXY_CONFIG_GET_BOOL("SUBGROUPING");
}
...
void FileDef::writeDetailedDescription(OutputList &ol,const QCString &title)
{
  if (hasDetailedDescription())
  {
    ...
    ol.startTextBlock();
    if (!briefDescription().isEmpty() && DOXY_CONFIG_GET_BOOL("REPEAT_BRIEF"))
    {
    ...
    if (DOXY_CONFIG_GET_BOOL("SOURCE_BROWSER")) 
    {
      //if Latex enabled and LATEX_SOURCE_CODE isn't -> skip, bug_738548
      ol.pushGeneratorState();
      if (ol.isEnabled(OutputGenerator::Latex) && !DOXY_CONFIG_GET_BOOL("LATEX_SOURCE_CODE"))
      { 
        ol.disable(OutputGenerator::Latex);
      }
      if (ol.isEnabled(OutputGenerator::RTF) && !DOXY_CONFIG_GET_BOOL("RTF_SOURCE_CODE"))
      { 
        ol.disable(OutputGenerator::RTF);
      }
...
{
  if (DOXY_CONFIG_GET_BOOL("HAVE_DOT") /*&& Config_getBool("INCLUDE_GRAPH")*/)
  {
...
{
  if (DOXY_CONFIG_GET_BOOL("HAVE_DOT") /*&& Config_getBool("INCLUDED_BY_GRAPH")*/)
  {
    //printf("Graph for file %s\n",name().data());
...
void FileDef::writeSource(OutputList &ol,bool sameTu,QStrList &filesInSameTu)
{
  ...
  static bool latexSourceCode   = DOXY_CONFIG_GET_BOOL("LATEX_SOURCE_CODE");
  static bool rtfSourceCode     = DOXY_CONFIG_GET_BOOL("RTF_SOURCE_CODE");
...
void FileDef::insertClass(ClassDef *cd)
{
  ...
  if (DOXY_CONFIG_GET_BOOL("SORT_BRIEF_DOCS"))
  {
...
{ 
  static bool sourceBrowser = DOXY_CONFIG_GET_BOOL("SOURCE_BROWSER");
  static bool verbatimHeaders = DOXY_CONFIG_GET_BOOL("VERBATIM_HEADERS");
...
{
  QCString vercmd = DOXY_CONFIG_GET_STRING("FILE_VERSION_FILTER");
...
{
  static bool optVhdl = DOXY_CONFIG_GET_BOOL("OPTIMIZE_OUTPUT_VHDL");
