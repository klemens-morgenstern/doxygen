...
{
  static QCString plantumlJarPath = DOXY_CONFIG_GET_STRING("PLANTUML_JAR_PATH");

  QCString pumlExe = "java";
  QCString pumlArgs = "";

  QStrList &pumlIncludePathList = DOXY_CONFIG_GET_LIST("PLANTUML_INCLUDE_PATH");
  ...
  else if (DOXY_CONFIG_GET_BOOL("DOT_CLEANUP"))
  {
    QFile(QCString(baseName)+".pu").remove();
  }
  portable_sysTimerStop();
  if ( (format==PUML_EPS) && (DOXY_CONFIG_GET_BOOL("USE_PDFLATEX")) )
  {
