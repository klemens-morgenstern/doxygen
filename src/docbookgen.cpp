...
void writeDocbookCodeBlock(FTextStream &t,FileDef *fd)
{
  ...
  pIntf->parseCode(*docbookGen,  // codeOutIntf
      0,           // scopeName
      fileToString(fd->absFilePath(),DOXY_CONFIG_GET_BOOL("FILTER_SOURCE_FILES")),
      langExt,     // lang
    ...
    if (md->memberType()==MemberType_Enumeration)
    {
      ...
      if (enumFields!=0) 
      {
        ...
        for (emli.toFirst();(emd=emli.current());++emli) 
        {
          ...
          t << "</term>" << endl;
          t << "                            <listitem>" << endl;
          if(DOXY_CONFIG_GET_BOOL("REPEAT_BRIEF"))
          {
              t << "                                <para>";
    ...
    bool detailed=0, const char *header=0,const char *documentation=0)
{
  ...
  if (detailed) 
  {
    for (mli.toFirst();(md=mli.current());++mli)
    {
        if (md->documentation().isEmpty() && !DOXY_CONFIG_GET_BOOL("REPEAT_BRIEF"))
        {
            continue;
        }
        doc_count = 1;
  ...
  for (mli.toFirst();(md=mli.current());++mli) 
  {
    ...
    if (d->definitionType()!=Definition::TypeFile || md->getNamespaceDef()==0) 
    {
        if (detailed && md->documentation().isEmpty() && !DOXY_CONFIG_GET_BOOL("REPEAT_BRIEF"))
        {
            continue;
        }
...
static void generateDocbookForClass(ClassDef *cd,FTextStream &ti)
{
  ...
  QCString outputDirectory = DOXY_CONFIG_GET_STRING("DOCBOOK_OUTPUT");
  ...
  if (DOXY_CONFIG_GET_BOOL("HAVE_DOT") && (DOXY_CONFIG_GET_BOOL("CLASS_DIAGRAMS") || DOXY_CONFIG_GET_BOOL("CLASS_GRAPH")))
  {
    t << "<para>Inheritance diagram for " << convertToXML(cd->name()) << "</para>" << endl;
    DotClassGraph inheritanceGraph(cd,DotNode::Inheritance);
    inheritanceGraph.writeGraph(t,GOF_BITMAP,EOF_DocBook,DOXY_CONFIG_GET_STRING("DOCBOOK_OUTPUT"),fileName,relPath,TRUE,FALSE);
  }

  if (DOXY_CONFIG_GET_BOOL("HAVE_DOT") && DOXY_CONFIG_GET_BOOL("COLLABORATION_GRAPH"))
  {
    t << "<para>Collaboration diagram for " << convertToXML(cd->name()) << "</para>" << endl;
    DotClassGraph collaborationGraph(cd,DotNode::Collaboration);
    collaborationGraph.writeGraph(t,GOF_BITMAP,EOF_DocBook,DOXY_CONFIG_GET_STRING("DOCBOOK_OUTPUT"),fileName,relPath,TRUE,FALSE);
  }
...
static void generateDocbookForFile(FileDef *fd,FTextStream &ti)
{
  ...
  {
    if (DOXY_CONFIG_GET_BOOL("INCLUDE_GRAPH"))
    {
      t << "<para>Include dependency diagram for " << convertToXML(fd->name()) << "</para>" << endl;
      DotInclDepGraph idepGraph(fd, FALSE);
      idepGraph.writeGraph(t,GOF_BITMAP,EOF_DocBook,DOXY_CONFIG_GET_STRING("DOCBOOK_OUTPUT"),fileName,relPath,FALSE);
    }
    if (DOXY_CONFIG_GET_BOOL("INCLUDED_BY_GRAPH"))
    {
      t << "<para>Included by dependency diagram for " << convertToXML(fd->name()) << "</para>" << endl;
      DotInclDepGraph ibdepGraph(fd, TRUE);
      ibdepGraph.writeGraph(t,GOF_BITMAP,EOF_DocBook,DOXY_CONFIG_GET_STRING("DOCBOOK_OUTPUT"),fileName,relPath,FALSE);
    }
  }
  ...
  if (DOXY_CONFIG_GET_BOOL("DOCBOOK_PROGRAMLISTING"))
  {
    t << "    <literallayout><computeroutput>" << endl;
...
static void generateDocbookForGroup(GroupDef *gd,FTextStream &ti)
{
  ...
  if (DOXY_CONFIG_GET_BOOL("GROUP_GRAPHS") && DOXY_CONFIG_GET_BOOL("HAVE_DOT"))
  {
    t << "<para>Collaboration diagram for " << convertToXML(gd->groupTitle()) << "</para>" << endl;
    DotGroupCollaboration collaborationGraph(gd);
    collaborationGraph.writeGraph(t,GOF_BITMAP,EOF_DocBook,DOXY_CONFIG_GET_STRING("DOCBOOK_OUTPUT"),fileName,relPath,FALSE);
  }
...
static void generateDocbookForDir(DirDef *dd,FTextStream &ti)
{
  ...
  t << "</title>" << endl;
  if (DOXY_CONFIG_GET_BOOL("DIRECTORY_GRAPH") && DOXY_CONFIG_GET_BOOL("HAVE_DOT"))
  {
    t << "<para>Directory dependency diagram for " << convertToXML(dd->displayName()) << "</para>" << endl;
    DotDirDeps dirdepGraph(dd);
    dirdepGraph.writeGraph(t,GOF_BITMAP,EOF_DocBook,DOXY_CONFIG_GET_STRING("DOCBOOK_OUTPUT"),fileName,relPath,FALSE);
  }
...
void generateDocbook()
{
  ...
  QCString dbk_projectName = DOXY_CONFIG_GET_STRING("PROJECT_NAME");
  QFile f(fileName);
