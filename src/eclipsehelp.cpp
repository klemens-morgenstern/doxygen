...
void EclipseHelp::initialize() 
{
  ...
  QCString name = DOXY_CONFIG_GET_STRING("HTML_OUTPUT") + "/toc.xml";
  ...
  QCString title = DOXY_CONFIG_GET_STRING("PROJECT_NAME");
  if (title.isEmpty())
  {
...
void EclipseHelp::finalize() 
{
  ...
  QCString name = DOXY_CONFIG_GET_STRING("HTML_OUTPUT") + "/plugin.xml";
  QFile pluginFile(name);
  if (pluginFile.open(IO_WriteOnly))
  {
    QString docId = DOXY_CONFIG_GET_STRING("ECLIPSE_DOC_ID");
