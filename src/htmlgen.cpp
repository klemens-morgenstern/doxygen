...
{
  static bool externalSearch = DOXY_CONFIG_GET_BOOL("EXTERNAL_SEARCH");
  t << "        <div id=\"MSearchBox\" class=\"MSearchBoxInactive\">\n";
  t << "          <div class=\"left\">\n";
                                       ...
                                       const QCString &navPath=QCString())
{
  ...
  static bool timeStamp = DOXY_CONFIG_GET_BOOL("HTML_TIMESTAMP");
  static bool treeView = DOXY_CONFIG_GET_BOOL("GENERATE_TREEVIEW");
  ...
  static bool mathJax = DOXY_CONFIG_GET_BOOL("USE_MATHJAX");
  static QCString mathJaxFormat = DOXY_CONFIG_GET_ENUM("MATHJAX_FORMAT");
  static bool disableIndex = DOXY_CONFIG_GET_BOOL("DISABLE_INDEX");
  static bool hasProjectName = !projectName.isEmpty();
  static bool hasProjectNumber = !DOXY_CONFIG_GET_STRING("PROJECT_NUMBER").isEmpty();
  static bool hasProjectBrief = !DOXY_CONFIG_GET_STRING("PROJECT_BRIEF").isEmpty();
  static bool hasProjectLogo = !DOXY_CONFIG_GET_STRING("PROJECT_LOGO").isEmpty();
  static bool titleArea = (hasProjectName || hasProjectBrief || hasProjectLogo || (disableIndex && searchEngine));

  cssFile = DOXY_CONFIG_GET_STRING("HTML_STYLESHEET");
  if (cssFile.isEmpty())
  {
    cssFile = "doxygen.css";
  }
  ...
  extraCssText = "";
  extraCssFile = DOXY_CONFIG_GET_LIST("HTML_EXTRA_STYLESHEET");
  ...
  if (timeStamp) 
  {
    generatedBy = theTranslator->trGeneratedAt(dateToString(TRUE), convertToHtml(DOXY_CONFIG_GET_STRING("PROJECT_NAME")));
  }
  else 
  {
  ...
  if (mathJax)
  {
    QCString path = DOXY_CONFIG_GET_STRING("MATHJAX_RELPATH");  
    ...
    QStrList &mathJaxExtensions = DOXY_CONFIG_GET_LIST("MATHJAX_EXTENSIONS");
  ...
  QCString result = substituteKeywords(s,title,
        convertToHtml(DOXY_CONFIG_GET_STRING("PROJECT_NAME")),
        convertToHtml(DOXY_CONFIG_GET_STRING("PROJECT_NUMBER")),
        convertToHtml(DOXY_CONFIG_GET_STRING("PROJECT_BRIEF")));
...
{
  dir=DOXY_CONFIG_GET_STRING("HTML_OUTPUT");
...
void HtmlGenerator::init()
{
  ...
  //writeLogo(dname);
  if (!DOXY_CONFIG_GET_STRING("HTML_HEADER").isEmpty()) 
  {
    g_header=fileToString(DOXY_CONFIG_GET_STRING("HTML_HEADER"));
    //printf("g_header='%s'\n",g_header.data());
  }
  else 
  {
  ...
  if (!DOXY_CONFIG_GET_STRING("HTML_FOOTER").isEmpty()) 
  {
    g_footer=fileToString(DOXY_CONFIG_GET_STRING("HTML_FOOTER"));
    //printf("g_footer='%s'\n",g_footer.data());
  }
  else 
  {
  ...
  {
    if (!DOXY_CONFIG_GET_STRING("MATHJAX_CODEFILE").isEmpty()) 
    {
      g_mathjax_code=fileToString(DOXY_CONFIG_GET_STRING("MATHJAX_CODEFILE"));
  ...
  if (DOXY_CONFIG_GET_BOOL("INTERACTIVE_SVG"))
  {
...

  {
    ...
    if (f.open(IO_WriteOnly))
    {
      ...
      if (DOXY_CONFIG_GET_BOOL("SOURCE_BROWSER") && DOXY_CONFIG_GET_BOOL("SOURCE_TOOLTIPS"))
      {
        t << endl <<
          "$(document).ready(function() {\n"
...
QCString HtmlGenerator::writeLogoAsString(const char *path)
{
  ...
  if (timeStamp)
  {
    result += theTranslator->trGeneratedAt(
               dateToString(TRUE),
               DOXY_CONFIG_GET_STRING("PROJECT_NAME")
              );
  }
  else
  {
...
void HtmlGenerator::writeStyleInfo(int part)
{
  ...
  if (part==0)
  {
    if (DOXY_CONFIG_GET_STRING("HTML_STYLESHEET").isEmpty()) // write default style sheet
    {
    ...
    {
      QCString cssname=DOXY_CONFIG_GET_STRING("HTML_STYLESHEET");
      QFileInfo cssfi(cssname);
      if (!cssfi.exists() || !cssfi.isFile() || !cssfi.isReadable())
      {
        err("style sheet %s does not exist or is not readable!", DOXY_CONFIG_GET_STRING("HTML_STYLESHEET").data());
      }
      else
      {
    ...
    static QStrList extraCssFile = DOXY_CONFIG_GET_LIST("HTML_EXTRA_STYLESHEET");
    for (uint i=0; i<extraCssFile.count(); ++i)
    {
      QCString fileName(extraCssFile.at(i));
      if (!fileName.isEmpty())
                               ...
                               const QCString &relPath,int sectionCount)
{
  ...
  static bool dynamicSections = DOXY_CONFIG_GET_BOOL("HTML_DYNAMIC_SECTIONS");
  if (dynamicSections)
  {
...
{
  bool generateLegend = DOXY_CONFIG_GET_BOOL("GENERATE_LEGEND");
  bool umlLook = DOXY_CONFIG_GET_BOOL("UML_LOOK");
  endSectionHeader(t);
  startSectionSummary(t,m_sectionCount);
  endSectionSummary(t);
  startSectionContent(t,m_sectionCount);
...
{
  static bool generateTreeView = DOXY_CONFIG_GET_BOOL("GENERATE_TREEVIEW");
  QCString fileName = DOXY_CONFIG_GET_STRING("HTML_OUTPUT")+"/search"+Doxygen::htmlFileExtension;
  QFile f(fileName);
  if (f.open(IO_WriteOnly))
  {
    FTextStream t(&f);
  ...
  QCString scriptName = DOXY_CONFIG_GET_STRING("HTML_OUTPUT")+"/search/search.js";
  QFile sf(scriptName);
  if (sf.open(IO_WriteOnly))
  {
    FTextStream t(&sf);
    ...
    t << "var serverUrl=\"" << DOXY_CONFIG_GET_STRING("SEARCHENGINE_URL") << "\";" << endl;
    t << "var tagMap = {" << endl;
    bool first=TRUE;
    // add search mappings
    QStrList &extraSearchMappings = DOXY_CONFIG_GET_LIST("EXTRA_SEARCH_MAPPINGS");
