...
{
  static QCString dotFontName = DOXY_CONFIG_GET_STRING("DOT_FONTNAME");
...
{
  static int dotFontSize = DOXY_CONFIG_GET_INT("DOT_FONTSIZE");
...
{
  static bool interactiveSVG = DOXY_CONFIG_GET_BOOL("INTERACTIVE_SVG");
  t << "digraph ";
  if (title.isEmpty())
  {
    t << "\"Dot Graph\"";
  ...
  if (DOXY_CONFIG_GET_BOOL("DOT_TRANSPARENT"))
  {
...
static void setDotFontPath(const char *path)
{
  ...
  QCString newFontPath = DOXY_CONFIG_GET_STRING("DOT_FONTPATH");
  QCString spath = path;
...
{
  static bool dotCleanUp = DOXY_CONFIG_GET_BOOL("DOT_CLEANUP"); 
  if (dotCleanUp)
  {
    QDir d;
    d.remove(dotName);
...
DotRunner::DotRunner(const QCString &file,const QCString &path,
  ...
  : m_dotExe(DOXY_CONFIG_GET_STRING("DOT_PATH")+"dot"),
    m_file(file), m_path(path), 
    m_checkResult(checkResult), m_imageName(imageName),
    m_imgExt(getDotImageExtension())
{
  static bool dotCleanUp      = DOXY_CONFIG_GET_BOOL("DOT_CLEANUP"); 
  static bool dotMultiTargets = DOXY_CONFIG_GET_BOOL("DOT_MULTI_TARGETS");
...
DotManager::DotManager() : m_dotMaps(1009)
{
  ...
  int i;
  int numThreads = QMIN(32,DOXY_CONFIG_GET_INT("DOT_NUM_THREADS"));
  if (numThreads!=1)
  {
...
bool DotManager::run()
{
  ...
  {
    setDotFontPath(DOXY_CONFIG_GET_STRING("HTML_OUTPUT"));
    setPath=TRUE;
  }
  else if (DOXY_CONFIG_GET_BOOL("GENERATE_LATEX"))
  {
    setDotFontPath(DOXY_CONFIG_GET_STRING("LATEX_OUTPUT"));
    setPath=TRUE;
  }
  else if (DOXY_CONFIG_GET_BOOL("GENERATE_RTF"))
  {
    setDotFontPath(DOXY_CONFIG_GET_STRING("RTF_OUTPUT"));
    setPath=TRUE;
  }
  ...
  if (ml)
  {
    ...
    for (mlia.toFirst();(mma = mlia.current());++mlia)
    {
      if (mma->getClassDef() == scope &&
          (skipNames==0 || skipNames->find(mma->name())==0))
      {
        static int limit = DOXY_CONFIG_GET_INT("UML_LIMIT_NUM_FIELDS");
                       ...
                       bool reNumber)
{
  ...
  {
    static bool dotTransparent = DOXY_CONFIG_GET_BOOL("DOT_TRANSPARENT");
    if (!dotTransparent)
    {
       ...
       const char *path,const char *fileName,int id) const
{
  ...
  QCString imgFmt = DOXY_CONFIG_GET_ENUM("DOT_IMAGE_FORMAT");
...
{
  if (DOXY_CONFIG_GET_BOOL("HIDE_UNDOC_CLASSES") && !cd->isLinkable()) return;
  ...
  else // new class
  {
    ...
    if (DOXY_CONFIG_GET_BOOL("HIDE_SCOPE_NAMES")) displayName=stripScope(displayName);
    QCString tmp_url;
    if (cd->isLinkable() && !cd->isHidden()) 
    {
      tmp_url=cd->getReference()+"$"+cd->getOutputFileBase();
                                          ...
                                          int maxNodes,bool includeParents)
{
  ...
  {
    static int maxDistance = DOXY_CONFIG_GET_INT("MAX_DOT_GRAPH_DEPTH");
  ...
  if (DOXY_CONFIG_GET_BOOL("UML_LOOK")) return FALSE; // UML graph are always top to bottom
  int maxWidth=0;
...
{
  static bool templateRelations = DOXY_CONFIG_GET_BOOL("TEMPLATE_RELATIONS");
...
DotClassGraph::DotClassGraph(ClassDef *cd,DotNode::GraphType t)
{
  ...
  static int maxNodes = DOXY_CONFIG_GET_INT("DOT_GRAPH_MAX_NODES");
...
void DotInclDepGraph::buildGraph(DotNode *n,FileDef *fd,int distance)
{
  ...
  if (includeFiles)
  {
    ...
    for (;(ii=ili.current());++ili)
    {
      ...
      if (doc || src || !DOXY_CONFIG_GET_BOOL("HIDE_UNDOC_RELATIONS"))
      {
        QCString url="";
...
DotInclDepGraph::DotInclDepGraph(FileDef *fd,bool inverse)
{
  ...
  static int nodes = DOXY_CONFIG_GET_INT("DOT_GRAPH_MAX_NODES");
  int maxNodes = nodes;
  //int directChildNodes = 1;
  //if (m_startNode->m_children!=0) 
  //  directChildNodes+=m_startNode->m_children->count();
...
void generateGraphLegend(const char *path)
{
  ...
  if (imgExt=="svg")
  {
    DotManager::instance()->addSVGObject(
        absBaseName+DOXY_CONFIG_GET_STRING("HTML_FILE_EXTENSION"),
        "graph_legend",
        absImgName,QCString());
  }
                           ...
                           const char *outFile,GraphOutputFormat format)
{
  ...
  {
    if (DOXY_CONFIG_GET_BOOL("USE_PDFLATEX"))
    {
